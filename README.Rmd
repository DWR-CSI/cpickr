...
-    `write_integra_files()`: Write a semi-colon delimited file format and a TSV summary file for the Integra Assist Plus robot cherrypicking operations.
...
For the Integra Assist Plus robot, the `write_integra_files()` function is used. It automatically manages deck constraints (a maximum of 96 samples per target plate, and a configurable maximum of unique source plates per run, e.g. 3).

```{r integra example, eval = FALSE}
...
This will write individual semi-colon delimited run files (`integra_1.csv`, `integra_2.csv`, etc.) and a tab-separated summary file `integra_summary.tsv` detailing each round on the robot.
